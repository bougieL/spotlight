...
#[tauri::command]
fn save_pasted_file(data_url: String, file_name: String) -> Result<String, String> {
    // Parse data URL: data:application/octet-stream;base64,<base64_data> or similar
    let parts: Vec<&str> = data_url.splitn(2, ',').collect();
    if parts.len() != 2 {
        return Err("Invalid data URL format".to_string());
    }
